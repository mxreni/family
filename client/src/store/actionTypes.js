// user action types
export const GET_CURRENT_USER = "getCurrentUser";
export const REMOVE_USER = "removeUser";
export const UPDATE_USER = "updateUser";
export const DELETE_USER = "deleteUser";
export const LOGIN_TO_USER_ACCOUNT="loginUser";

// members action types
export const GET_MEMBERS = "getMembers";
export const ADD_MEMBERS = "pushMembers";
export const GET_MEMBER_BY_ID = "getMemberById";
export const EDIT_MEMBER_BY_ID = "editMemberById";
export const DELETE_MEMBER_BY_ID = "deleteMemberById";

// relationship action types
export const GET_RELATIONSHIPS = "getRelationships";
export const GET_RELATIONSHIP_BY_ID = "getRelationshipById";

// tree action types

export const GET_TREE = "getTree";
export const GET_TREE_BY_ID = "getTreeById";
export const ADD_TO_TREE = "pushToTree";
export const EDIT_TREE_BY_ID = "editTreeById";
export const DELETE_TREE_BY_ID = "deleteTreeById";
